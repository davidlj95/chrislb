...
export class Cloudinary implements ImageCdnApi {
  ...
  async getAllImagesInPath(
    path: string,
  ): Promise<readonly CloudinaryImageAsset[]> {
  ...
  private async _listImageAssetsInPath(
    ...
    const response = await cloudinary.api.resources_by_asset_folder(path, {
      ...
      resource_type: 'image',
      fields: 'width,height,version,tags', // public_id and asset_id are always included
      tags: true,
    })
    return response.resources
      .filter((resource) => !resource.tags.includes(UNPUBLISHED_TAG))
      .toSorted((a, b) => (a.public_id < b.public_id ? -1 : 1))
      .map(({ public_id, width, height, version, asset_id }) => ({
        filename: public_id,
        width,
        height,
        version,
